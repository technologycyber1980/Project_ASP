...
switch(true){
	...
	case (isset($order_id_keypass)):
		...
		$sql_order_no= " SELECT order_id,order_no,so_finish_status FROM new_po WHERE order_no='".decode($order_id_keypass)."' LIMIT 1 ";	
	...
	case (isset($oid)):
		...
		$sql_order_no= " SELECT order_id,order_no,so_finish_status FROM new_po WHERE order_no='$oid' LIMIT 1 ";	
		$con_order_no= mysql_query($sql_order_no);
		$rs_order_no= mysql_fetch_array($con_order_no);
	...
	case (isset($fm_car_no_keypass)):/* FM CAR. */
		...
		WHERE asp_fm_qc_011_car.fm_car_id='".decode($fm_car_no_keypass)."'
...
$num_of_photo= mysql_num_rows($con_pic);
...
<html>
  <head>
	...
	<title>PHOTO Gallery<? echo ' : '.$rs_pic[doc_no];?></title>
...
<div class="contain-view-card"	style=""	 >
  ...
  <div class="headder" style="">PHOTO Gallery<br><span><? echo $rs_pic[doc_no];?></span></div>
    ...
    <div class="card" data-tilt data-tilt-scale=""  style="">
	<? }else{ ?>	
...
while($rs_pic= mysql_fetch_array($con_pic)){	
		...
		if((date('Y-m-d H:i:s') < date('Y-m-d H:i:s',strtotime("+1 day",strtotime( $rs_pic[create_date]))))){
		$show_button_delete_edit_pic=true;
		}else{		
		$show_button_delete_edit_pic=false;	
		}	
	
	$ex_pic_type=explode('/',$rs_pic[pic_type]);?>
	
<? if(filesize($rs_pic[pic]) > 0 ){ ?>
    <div class="card rgb" data-tilt data-tilt-scale="1.1" style="">
		
	<?										
	switch($ex_pic_type[0]){
		case "video": ?>
		<form action="player.php?top_bar=enable&autoplay=enable&order_media_id_keypass=<? echo encode($rs_pic[id]); ?>"  enctype="multipart/form-data" method="post"
			name="click_<? echo $rs_pic[id];?>"
			class="card-image" 
			align="center"  
			style="
			cursor:pointer;
	  ...
	  <video
	style=""
			 autoplay loop muted src="<? echo $rs_pic[pic]; ?>" height="100%"></video>
		<? 
		break; 
		...
		case "image":?>
			...
			class="card-image" 
			style="
		   	background:linear-gradient(#fff0 0%, #fff0 70%, #1d1d1d 100%),url('<? echo $rs_pic[pic_thumb]; ?>') no-repeat center;
		   	cursor:pointer;
			background-size:cover;"  
			<? if($device_platform=="MOBILE"){?>
			onclick="javascript:window.open('<? echo $rs_pic[pic]; ?>','_parent');"
			<? } else { ?>			
			onclick="this.submit()"	  
			<? } ?>	  
		...
		case "application":	
			...
			switch(true){
					...
					?>
			<form action="<? echo $rs_pic[pic]; ?>"  enctype="multipart/form-data" method="post"
			class="card-image" 
			name="click_<? echo $rs_pic[id];?>"
			style="
			background:linear-gradient(#fff0 0%, #fff0 70%, #1d1d1d 100%),url('<? echo "../image_background/doc_pdf.svg"; ?>') no-repeat center;background-size:cover;cursor:pointer;"
			onclick="this.submit()" >
					<?
					break;
					default:
			...
			class="card-image" 
			style="
			background:linear-gradient(#fff0 0%, #fff0 70%, #1d1d1d 100%),url('<? echo "../image_background/doc_all.svg"; ?>') no-repeat center;background-size:cover;cursor:pointer;"
			onclick="this.submit()" >
		...
		<input type="hidden" name="create_date" value='<? echo strtotime($rs_pic[create_date]); ?>'></input>
		<input type="hidden" name="create_by" value='<? echo $rs_pic[create_by]; ?>'></input>
		<input type="hidden" name="pic_path" value='<? echo $rs_pic[pic]; ?>'></input>
		<input type="hidden" name="pic_title" value='<? echo $rs_pic[pic_desc]; ?>'></input>
	  </form>
	...
	switch($ex_pic_type[0]){
		...
		<div class="card-stats">
			<div class="stat">
				<!-- DELETE BUTTON -->
				<? if($display!="none" && $show_button_delete_edit_pic){?>
	...
	switch($ex_pic_type[0]){
		...
		background-size:auto auto;
		cursor:pointer;"  
		onclick="javascript:window.open('player.php?top_bar=enable&autoplay=enable&order_media_id_keypass=<? echo encode($rs_pic[id]); ?>','_parent');"></div>		
		<? 
		break;
		case "image":
		?>
	  ...
	  <? }else{ ?>
				<form  action="display.php" enctype="multipart/form-data" method="post" name="form_display_<? echo $rs_pic[id]?>">
	  ...
	  <? } ?>
		...
		width:55px;
		height:55px;
		background:url('../image_icon/doc_view_icon.svg') no-repeat center;
		background-size:auto auto;
		cursor:pointer;" 
		onclick="javascript:window.open('<? echo $rs_pic[pic]; ?>','_parent');"></div>
	<? 
